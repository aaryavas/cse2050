class CustomSet:
    ...
    def remove(self, item):
        ...
        if item not in self:
            raise KeyError(f'{item} does not exist in the CustomSet.')
        # Find index of bucket `item` is in (self._find_bucket())
        m = self._find_bucket(item)
        # Remove item from bucket
        self._L[m].remove(item)
        # update length
        self._len -= 1
    ...
    def _rehash(self, new_buckets):
        ...
        for bucket in self._L:
            ...
            # using a for loop, iterate over every item in this bucket
                ...
                new_bucketidx = hash(item) % new_buckets
                # add that item to the correct bucket
                new_L[new_bucketidx].append(item)
